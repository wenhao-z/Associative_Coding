function hFig = contourAssoFields_cMap(BMStruct, hFig)
...
cmap = getColorMapPosNegDat([min(WPlot(:)), max(WPlot(:))]);
colormap(hAxe(1), cmap);
...
cAxis = [min(reshape(WPlot(:,:,1), 1,[])), ...
    max(reshape(WPlot(:,:,1), 1,[]))];
cmap = getColorMapPosNegDat(cAxis);
colormap(hAxe(2), cmap)
...
cAxis = [min(reshape(WPlot(:,:,2), 1,[])), ...
    max(reshape(WPlot(:,:,2), 1,[]))];
cmap = getColorMapPosNegDat(cAxis);
colormap(hAxe(3), cmap)
