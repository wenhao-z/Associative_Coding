function hFig = plotAssoFields(BMStruct, hFig)
...
cmLength = 128;
% cMap = jet(cmLength);
[cMap, cIdxZero, colorTick] = getColorMapPosNegDat([cMin, cMax], cmLength);
colormap(cMap);
...
for iterOrient = 1: szIdxNeuron(2)
    for IdxDim3 = 1: szIdxNeuron(3) % height
        for IdxDim4 = 1: szIdxNeuron(4) % width
            ...
            if ~isnan(WPlot(1, iterOrient, IdxDim3, IdxDim4))
                cSpec = find( (WPlot(1, iterOrient, IdxDim3, IdxDim4)-colorTick)>=0, 1, 'last');
            else
                cSpec = cIdxZero;
            end
%             [~, cSpec] = min(abs(WPlot(1, iterOrient, IdxDim3, IdxDim4) - colorTick));
%             cSpec = fix((WPlot(1, iterOrient, IdxDim3, IdxDim4)-cMin)/(cMax-cMin)*cmLength)+1;
            cSpec = cMap(min(cSpec, cmLength), :);
            
            lineWidth = 15 * abs(WPlot(1, iterOrient, IdxDim3, IdxDim4));
...
xlabel('Horizontal location of hypercolumns')
ylabel('Vertical location of hypercolumns')
colorbar('Ticks', [0, cIdxZero/cmLength, 1],...
